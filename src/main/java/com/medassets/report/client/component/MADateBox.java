package com.medassets.report.client.component;

import com.google.gwt.i18n.client.DateTimeFormat;
import java.util.Date;

import com.google.gwt.user.client.ui.Label;
import com.google.gwt.user.datepicker.client.DateBox;

public class MADateBox extends DateBox implements MAComponent {

    Label label;
    Object dto;
    boolean required;

    public MADateBox() {
        label = new Label();
        setWidth("100%");
        setFormat(new DateBox.DefaultFormat(DateTimeFormat.getFormat("MM/dd/yyyy")));
//            DateTimeFormat.PredefinedFormat.DATE_SHORT)));
        getTextBox().setValue("mm/dd/yy");
    }

    @Override
    public void setMAValue(Object val) {
        this.setValue(val == null ? null : new Date(val.toString()));
    }

    @Override
    public Object getMAValue() {
        return this.getValue();
    }

    @Override
    public void setMALabel(String text) {
        this.label.setText(text);
    }

    @Override
    public Label getMALabel() {
        return this.label;
    }

    public void setDTO(Object dto) {
        this.dto = dto;
    }

    public Object getDTO() {
        return this.dto;
    }

    public void setMATooltip(String tooltip) {
        label.setTitle(tooltip);
        this.setTitle(tooltip);
    }

    public void setMARequired(boolean b) {
        this.required = b;
    }

    public boolean isMARequired() {
        return this.required;
    }

    public boolean isMAEmpty() {
        return this.getValue() == null;
    }

    public void setMAFocus() {
        this.setFocus(true);
    }

    public void setMAEnabled(boolean b) {
        this.setEnabled(b);
    }
}
